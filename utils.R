...
datos_min_ccaa_col <- function(raw_datos_min, tgt_var,
  ...
  datos_min <- datos_min %>% 
    mutate(ES = rowSums(datos_min %>% select_if(is.numeric)))
...
datos_min_ccaa <- function(raw_datos_min, cod_ccaa, perc_100K = TRUE) {
  
  if(cod_ccaa == "ES") {
    out <- raw_datos_min %>% 
      group_by(fecha) %>% 
      summarise_if(is.numeric, sum, na.rm = TRUE)
    out <- tibble(ccaa_codigo_iso_ccaa = rep("ES", nrow(out)),
